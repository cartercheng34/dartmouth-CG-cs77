...
// ################ Edit your code below
    ...
    modelViewProjectionMatrix = modelViewProjectionMatrix.multiply(projection).multiply(tmp).multiply(model);
    // modelViewProjectionMatrix = modelViewProjectionMatrix.multiply(tmp);
    // modelViewProjectionMatrix = modelViewProjectionMatrix.multiply(projection);
    // alert(modelViewProjectionMatrix.m);
    // ################
    ...
    for (var i = 0; i < this.indices.length; i+=2)
    {
        ...
        // TODO: Modify your render implementation from before to apply the model-view-projection
        //       transform matrix to every vertex before projection.

        xyz1 = modelViewProjectionMatrix.multiplyVector(xyz1);
        xyz2 = modelViewProjectionMatrix.multiplyVector(xyz2);

// ################ Edit your code below
        var xy1 = [xyz1[0] / xyz1[2], xyz1[1] / xyz1[2]];
        var xy2 = [xyz2[0] / xyz2[2], xyz2[1] / xyz2[2]];
// ################
