function catmullClarkSubdivisionHard(vertices, faces, sharpness, hardVertices, hardFaces) {
    var newVertices = [];
    var newFaces = [];
    // var hardFaces = [];
	var newHardVertices = [];
    ...
    function getOrInsertEdge(a, b, centroid, i) {
        ...
        if (edgeKey in edgeMap) {
            ...
            if(hardVertices[a] == 1.0 && hardVertices[b])
                newHardVertices.push(1.0);
            else newHardVertices.push(0);
            edgeMap[edgeKey] = idx;
            return idx;
        }
    }
    ...
    //init newVertices for drawing

    for(var i = 0 ; i < vertices.length ; i++){
        newVertices.push(vertices[i].clone());
        newHardVertices.push(hardVertices[i]);
	}
    ...
    for(var i = 0 ; i < faces.length ; i++){
        ...
        for(var j = 0 ; j <faces[i].length ; j++){
            ...
            var edgePointA = getOrInsertEdge(v0, v1, centroid_01, i);
            var edgePointB = getOrInsertEdge(v1, v2, centroid_12, i);
            newFaces.push([centroid_idx, edgePointA, v1, edgePointB]);
			if(hardFaces[i] == 1.0)
				newHardFaces.push(1.0);
			else newHardFaces.push(0.0);
        }
	}
	// alert(newVertices.length);

    //step 2, compute displacement for soft vertices
    var avgV = []
    var avgN = []
    ...
    //step 3 smooth, if vertices are targeted as hard, then do not move
    
    for(var i = 0 ; i < avgV.length ; i++){
        if(newHardVertices[i] <= 0 || sharpness <= 0)
	...
	var new_mesh = new Mesh(newVertices, newFaces);
	new_mesh.setHard(newHardFaces, newHardVertices);

    return new_mesh;
};

function extraCreditMesh2() {
...
Task4.prototype.setSubdivisionLevel = function(subdivisionLevel) {
    this.subdivisionLevel = subdivisionLevel;
    this.computeMesh();
}

Task4.prototype.setSharpness = function(sharpness) {
    this.sharpness = sharpness;
    this.computeMesh();
}

Task4.prototype.selectModel = function(idx) {
    this.selectedModel = idx;
    this.computeMesh();
}

Task4.prototype.baseMesh = function(modelIndex) {
    switch(modelIndex) {
    ...
    case 5: return extraCreditMesh2(); break;
    }
    return null;
}

Task4.prototype.computeMesh = function() {
    ...
    for (var i = 0; i < this.subdivisionLevel; ++i){
        mesh = catmullClarkSubdivisionHard(mesh.vertices, mesh.faces, sharpness, mesh.hardVertices, mesh.hardFaces);
        sharpness--;
    }
