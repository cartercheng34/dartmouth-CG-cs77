var Mesh = function(vertices, faces, joinVerts) {
    ...
    for(var i = 0 ; i < vertices.length ; i++){
        this.hardVertices.push(0);
    }

    for(var i = 0 ; i < faces.length ; i++){
        if(i == 2 ){
            this.hardFaces.push(1.0);
            for (var j = 0 ; j<faces[i].length ; j++){
                this.hardVertices[faces[i][j]] = 1.0;
            }
        }
		else this.hardFaces.push(0.0);
    }
}

Mesh.prototype.setHard = function(hardFaces, hardVertices) {
    this.hardFaces = hardFaces;
    this.hardVertices = hardVertices;
}
